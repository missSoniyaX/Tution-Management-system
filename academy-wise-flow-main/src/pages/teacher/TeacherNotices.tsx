...
export default function TeacherNotices() {
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      ...
      await createNotice.mutateAsync({ ...form, created_by_name: user?.name || "Teacher" });
          ...
          {notices.map((n) => (
            <div key={n.id} className={`bg-card rounded-xl border p-5 shadow-card ${n.is_emergency ? "border-destructive/30" : "border-border"}`}>
              <div className="flex justify-between items-start">
                <div>
                  {n.is_emergency && <span className="inline-block bg-destructive/15 text-destructive text-xs font-semibold px-2 py-0.5 rounded-full mb-2">Emergency</span>}
                ...
                <div className="flex gap-1 ml-3">
                  ...
                  <button onClick={async () => { await deleteNotice.mutateAsync(n.id); toast({ title: "Deleted" }); }}
